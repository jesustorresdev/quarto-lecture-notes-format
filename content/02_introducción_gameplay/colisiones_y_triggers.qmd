...
La consulta por perfil se hace empleado funciones como **LineTraceByProfile** (`UWorld:: LineTraceSingleByProfile()` en C++), que también se puede observar en la @fig-line-trace.
...
Las funciones de ***raycast*** devuelven información sobre el impacto de la traza en el objeto, mediante la estructura `HitResult`.
Si no necesitamos esta información, sino que sólo interesa saber si hay un objeto bloqueado la traza, podemos usar una versión de estas funciones marcada con la palabra `Test`.
Por ejemplo, `UWorld::LineTraceTestForObject()` o `UWorld::LineTraceTestByChanneL()`.
...
|          +----------+-------------------------+------------------------------+
|          | Perfil   | LineTraceByProfile      | `LineTraceSingleByProfile()` |
+----------+----------+-------------------------+------------------------------+
|                                                                              |
+----------+----------+-------------------------+------------------------------+
| Multiple | Objeto   | MultiTraceForObject     | `LineTraceMultiForObject()`  |
|          +----------+-------------------------+------------------------------+
|          | Traza    | MultiTraceByChannel     | `LineTraceMultiByChannel()`  |
|          +----------+-------------------------+------------------------------+
|          | Perfil   | MultiTraceByProfile     | `LineTraceMultiByProfile()`  |
+----------+----------+-------------------------+------------------------------+
|                                                                              |
+----------+----------+-------------------------+------------------------------+
| Test     | Objeto   | N/A                     | `LineTraceTestForObject()`   |
|          +----------+-------------------------+------------------------------+
|          | Traza    | N/A                     | `LineTraceTestByChannel()`   |
|          +----------+-------------------------+------------------------------+
|          | Perfil   | N/A                     | `LineTraceTestByProfile()`   |
+----------+----------+-------------------------+------------------------------+
...
|          |                                                                       |
|          +----------+-----------------------------+------------------------------+
|          | Perfil   | BoxTraceByProfile\          | `SweepSingleByProfile()`     |
...
|          |                                                                       |
|          +----------+-----------------------------+------------------------------+
|          | Perfil   | MultiBoxTraceByProfile\     | `SweepMultiByProfile()`      |
|          |          | MultiSphereTraceByProfile\  |                              |
|          |          | MultiCapsuleTraceByProfile  |                              |
+----------+----------+-----------------------------+------------------------------+
|                                                                                  |
+----------+----------+-----------------------------+------------------------------+
| Test     | Objeto   | N/A                         |  `SweepTestForObject()`      |
|          |          |                             |                              |
|          |          |                             |                              |
|          +----------+-----------------------------+------------------------------+
|          |                                                                       |
|          +----------+-----------------------------+------------------------------+
|          | Traza    | N/A                         | `SweepTestByChannel()`       |
|          |          |                             |                              |
|          |          |                             |                              |
|          +----------+-----------------------------+------------------------------+
|          |                                                                       |
|          +----------+-----------------------------+------------------------------+
|          | Perfil   | N/A                         | `SweepTestByProfile()`       |
|          |          |                             |                              |
|          |          |                             |                              |
+----------+----------+-----------------------------+------------------------------+
...
También existe un grupo de funciones con los prefijos `ComponentOverlap` y `ComponentSweep` que permiten hacer consultas usando la malla de colisión de un componente dado.
Por ejemplo, la colisión simple configurada en un **StaticMesh**.

## Consultas de superposición
...
|          | **Capa** | ***Blueprints***               | **C++** (`UWorld::*`)                     |
+==========+==========+================================+===========================================+
| Multiple | Objeto   | BoxOverlapActors\              | `OverlapMultiByObjectType()`              |
|          |          | SphereOverlapActors\           |                                           |
...
|          |                                                                                       |
|          +----------+--------------------------------+-------------------------------------------+
|          | Traza    | N/A                            | `OverlapMultiByChannel()`                 |
|          |          |                                |                                           |
|          |          |                                |                                           |
|          +----------+--------------------------------+-------------------------------------------+
|          |                                                                                       |
|          +----------+--------------------------------+-------------------------------------------+
|          | Perfil   |                                | `OverlapMultiByProfile()`                 |
|          |          |                                |                                           |
|          |          |                                |                                           |
+----------+----------+--------------------------------+-------------------------------------------+
|                                                                                                  |
+----------+----------+--------------------------------+-------------------------------------------+
| Test     | Objeto   | N/A                            | `OverlapAnyTestByObjectType()`            |
|          |          |                                |                                           |
|          +----------+--------------------------------+-------------------------------------------+
|          |                                                                                       |
|          +----------+--------------------------------+-------------------------------------------+
|          | Traza    | N/A                            | `OverlapAnyTestByChannel()`\              |
|          |          |                                | `OverlapBlockingTestByChannel()`          |
|          +----------+--------------------------------+-------------------------------------------+
|          |                                                                                       |
|          +----------+--------------------------------+-------------------------------------------+
|          | Perfil   | N/A                            | `OverlapAnyTestByProfile()`               |
...
En la @tbl-async-raycast se puede ver una listado de todas ellas.

+---------------+----------+--------------------------------+
|               | **Capa** | **C++** (`UWorld::*`)          |
+===============+==========+================================+
| Sencillo      | Objeto   | `AsyncLineTraceByObjectType()` |
|               +----------+--------------------------------+
|               | Traza    | `AsyncLineTraceByChannel()`    |
|               +----------+--------------------------------+
|               | Perfil   | `AsyncLineTraceByProfile()`    |
+---------------+----------+--------------------------------+
|                                                           |
+---------------+----------+--------------------------------+
| Superposición | Objeto   | `AsyncOverlapByObjectType()`   |
|               +----------+--------------------------------+
|               | Traza    | `AsyncOverlapByChannel()`      |
|               +----------+--------------------------------+
|               | Perfil   | `AsyncOverlapByProfile()`      |
+---------------+----------+--------------------------------+
|                                                           |
+---------------+----------+--------------------------------+
| Forma         | Objeto   | `AsyncSweepByObjectType()`     |
|               +----------+--------------------------------+
|               | Traza    | `AsyncSweepByChannel()`        |
|               +----------+--------------------------------+
|               | Perfil   | `AsyncSweepByProfile()`        |
+---------------+----------+--------------------------------+
...
Pera esto último, tendremos que indicar dicha función a través de un parámetro de las funciones de ***raycast*** que espera un *delegado*.
