...
Por lo general, al salvar la partida es necesario almacenar el estado de multiples objetos en el nivel actual.
...
Esta interfaz debe tener métodos para guardar y cargar el estado del actor en alguna estructura de datos, y cada actor salvable debe implementar esta interfaz para guardar y cargar correctamne su estado en o desde dicha estructura, respectivamente.
...
Por ejemplo, si el actor es un caja que puede ser abierta, es probable que tenga una propiedad que indique si ya ha sido abierta por el jugador, y que cuando se restablezca el estado del actor desde la partida guardada, el actor use esta propiedad para cambiar su aspecto visual y mostrar la caja abierta.
...
Algunos de estos datos pueden ser el nombre u otro tipo de identficación del actor, su posición y rotación en el mundo, etc.
...
Pero, aún así, en actores complejos, la cantidad de propiedades a guardar puede ser muy grande, complicando el desarrollo y la mantenibilidad de los métodos de guardado y carga.
...
En condiciones normales, en la copia serializada se incluirán toda las propiedades de **AMyActor**, pero si el archivo donde se serializa tiene activada su propiedad **ArIsSaveGame**, solo se incluirán las propiedades del actor marcadas con el especificador **SaveGame**, como la propiedad **bIsActivated** del ejemplo.
...
1. Crear el objeto **SaveGame** a partir de una clase con las propiedades adecuadas para almacenar el estado global del juego.
   Entre esta información probablemente se deba incluir el nivel actual, la posición del lugador, el inventario o las habilidades, etc.
...
En ese sentido, usar previamente `LoadPackageAsync()` permite precargar el nivel de forma asíncrona --para mostrar mientras una pantalla de carga, haciendo que luego la carga del nivel con `OpenLevel()`sea inmedianta.
...
3. Restablecer los valores de las propiedades guardadas en el objeto **SaveGame** en el nivel cargado, puesto que la carga con `OpenLevel()` se realiza con los valores por defecto.
   ...
   Para que el **GameMode** sepa que tiene que cargar los datos de una partida salvada, se puede utilizar el argumento **Options** de `OpenLevel()`, que sirve para pasar información adicional al **GameMode** del nivel cargado en la forma de los parametros de una URL, como se puede observar el la @fig-openlevel-options.
...
![Argumentos de **OpenLevel**.](figures/openlevel-options.png){#fig-openlevel-options}
