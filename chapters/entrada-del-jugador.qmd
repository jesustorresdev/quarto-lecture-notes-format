---
abstract: |
  En este capítulo se explica cómo se lee la entrada del jugador en Unreal Engine y cómo se puede usar para controlar el movimiento y la rotación del personaje.
  Se explica cómo se puede configurar el sistema de entrada para que sea independiente del dispositivo de entrada y cómo se puede usar para controlar la cámara del jugador.
  También se explica cómo se puede implementar la interacción con objetos del escenario, de forma que el jugador pueda cogerlos o accionarlos.
---
...
## Sistema de entrada en Unreal Engine

Como hemos mencionado anteriormente, el control de personaje recibe la entrada del jugador para determinar el movimiento y las acciones que realiza el personaje.
...
Durante la ejecución, cualquier actor puede leer o suscribirse a estas acciones, para ser notificado en el momento en el que se dispare alguna.
Los actores reciben las acciones a través de un componente denominado **InputComponent**.

En el @vid-input-component-stack se puede ver como los **InputComponent** de los actores interesados en la entrada del jugador se insertan en una pila, que gestiona el actor **PlayerController**.
...
En el @vid-input-component-stack también se observa que, por defecto, el primer actor en la pila de **InputComponent** es el **PlayerController**, mientras que el siguiente es el **Pawn** del personaje del jugador.
...
Para conocer con más detalle cómo se gestiona la entrada en Unreal Engine se pueden consultar la @sec-controllers y la  @sec-playercontroller.
...
Por ejemplo, el **CharacterMovementComponent** espera que sea un vector normalizado con la dirección y sentido en la que acelerar al personaje, porque utiliza un control de movimiento de tercer orden.
...
![Uso de la entrada para el movimiento.](/assets/entrada-del-jugador/pawn-movement-input.png){#fig-pawn-movement-input}
...
![Uso de la entrada para actualizar la *rotación de control*.](/assets/entrada-del-jugador/control-rotation-input.png){#fig-control-rotation-input}
...
Por tanto, el valor resultado es independiente de los FPS a los que se ejecute el juego, pues a tasas más bajas las actualizaciones ocurren con menos frecuencia, pero el valor de desplazamiento acumulado final no cambia.
Esto significa que la lectura del ratón se puede acumular directamente en la *rotación de control*, generalmente escalada por alguna magnitud para ajustar el grado de sensibilidad del control.
En la @fig-camera-input-stick-imc-config se puede observar un ejemplo de esta configuración dónde se mapea el ratón a la acción **IA_Look**, simplemente añádiendo un modificar para escalar el valor de la entrada con el objeto de poder ajustar la sensbilidad.

Sin embargo, teclados y *sticks* devuelve valores entre 1.0 y -1.0.
Si se decidiese acumular directamente sus valores en la *rotación de control*, la velocidad de rotación dependería de los FPS a los que se ejecute el juego.
Por ejemplo, cuando la activación es máxima, en cada actualización se aplicaría sistemáticamente un grado.
Es decir, a más FPS, más grados por segundo se aplican.
...
Así, el control se hace independiente de los FPS a los que se ejecute el juego.
...
En ese caso, el valor del *stick* se toma como un vector velocidad angular normalizado, por lo que se añaden dos modificadores.
Uno para multiplicar por el `DeltaTime` y otro para escalar por la velocidad máxima de rotación.

![Ratón y *stick* mapeados a la acción **IA_Locok**.](/assets/entrada-del-jugador/camera-input-stick-imc-config.png){#fig-camera-input-stick-imc-config}

En la @fig-control-rotation-input se ve como los valores de la acción **IA_Look** se acumulan en la *rotación de control* directamente, sin importar el dispositivo de entrada, porque se han configurado los modificadores adecuados en el **Input Mapping Context**.

### Personaje en tercera persona con cámara orbital

Cuando el jugador maneja algún tipo de personaje humanoide se suele utilizar el actor {{< api Character >}} que incluye el componente de movimiento {{< api CharacterMovementComponent >}}.
...
![Opción para orientar el personaje en la dirección del movimiento.](/assets/entrada-del-jugador/orient-rotation-to-movement.png){#fig-orient-rotation-to-movement}
...
![Activar **Use Pawn Control Rotation** en el componente **SpringArm**.](/assets/entrada-del-jugador/spring-arm-pawn-control-rotation.png){#fig-spring-arm-pawn-control-rotation}

Como se explica en la @sec-camera-customization, se pueden usar un modificador de cámara o las propiedades del **PlayerCameraManager** para limitar los ángulos de rotación de la cámara.
Por ejemplo, para que la cámara no pueda colocarse completamente arriba mirando hacia abajo o abajo mirando hacia arriba.

Otro detalle importante es que, a diferencia de lo que se ve en la @fig-pawn-movement-input en la gestión de la entrada de movimiento, en este caso también se utiliza la *rotación de control* en el cálculo del vector de movimiento, porque el comando de movimiento del jugador se interpreta en la orientación de la cámara.
...
![Uso de la entrada para el movimiento en un juego en tercera persona.](/assets/entrada-del-jugador/tpp-pawn-movement-input.png){#fig-tpp-pawn-movement-input}

### Personaje en primera persona

En los juegos en primera persona la cámara suele estar fija en la cabeza del personaje.
En este caso, se puede activar **Use Pawn Control Rotation** en la **Camera** para que esta se oriente en la dirección de la *rotación de control*
...
En este último caso la rotación no tiene que ser instantánea, sino que se puede configurar una velocidad de rotación en la propiedad **Rotation Rate** (ver la @fig-orient-rotation-to-movement).
...
Los vehículos suelen tener algunas reglas respecto al funcionamiento de la dirección, de forma que no suelen poder cambiar de dirección libremente, sino que orientación y movimiento sobre la superficie están relacionadas.
...
En este caso, la *rotación de control* se suele usar para controlar la cámara, que suele ser libre, es decir, que puede mirar en cualquier dirección.

## Otras entradas

Como comentamos al principio, el controlador del personaje no solo se encaga del movimiento y la rotación, sino también de otras acciones que puede realizar el personaje.
...
* Si solo el personaje tuviera acceso a la entrada del jugador, probablemente implementar la funcionalidad en el propio personaje es la mejor o, quizás, la única opción.
  Pero en la arquitectura de Unreal Engine ilustrada en el @vid-input-component-stack, los objetos pueden leer la entrada del jugador si se colocan los primeros en la pila de **InputComponent** del **PlayerController**.
  
  Esto no ocurre en otros motores, por lo que tendríamos que implementar la funcionalidad en el personaje o desarrollar un mecanismo similar al que tiene Unreal Engine para que los objetos puedan leer la entrada del jugador.

* Si el propio objeto es quien tiene la responsabilidad de gestionar la interacción del jugador, el personaje no tiene que preocuparse de ello, lo que evita añadir esta responsabilidad en el código del personaje que, por lo general, ya tiene muchas otras cosas de las que encargarse.
De esta forma, puede ser más fácil añadir nuevos tipos de objetos con los que puede interactuar el jugador.

* Si se quiere una solución genérica, que permitan tanto al jugador como a los NPC interactuar con los objetos, es mejor que la lógica de interacción esté en el personaje, porque el mecanismo para leer la entrada del jugador no sirve con los NPC. 

Suponiendo que optamos porque el objeto sea quien gestione la interacción, la forma más sencilla de hacerlo es activar la entrada del jugador en el objeto cuando este entra en el *trigger* del objeto.
Para ello, se añade el **InputComponent** del actor del objeto a la pila del **PlayerController** del jugador, usando el método {{< api EnableInput >}} de la clase **Actor** del objeto.
Posteriormente, cuando el jugador se aleja del objeto, lo único que hay que hacer es quitar el **InputComponent** del actor de la pila del **PlayerController**, usando el método {{< api DisableInput >}}.
Ambos métodos necesitan como argumento el **PlayerController** del jugador del que quieren leer o dejar de leer la entrada.
...
%%| file: ../diagrams/entrada-del-jugador/object-interaction-sequence.mmd
```
...
Es buena idea evitar las dependencias directas entre actores.
...
Pero esto introduce una dependencia entre ambas clases, que puede limitarse usando una interfaz común para las puertas y otros objetos activables por los pulsadores.
...
Para que la puerta no tenga una dependencia con el pulsador para suscribirse, la conexión entre ambos puede hacer en el nivel, por ejemplo utilizando el el **Level Blueprint* (ver el @sec-level-blueprint).
...
Este último ángulo se puede obtener fácilmente usando {{< api FindLookAtRotation >}}.
...
![Determinar si el personaje mira al objeto.](/assets/entrada-del-jugador/angulo-de-vision.png){#fig-angulo-de-vision}
...
::: {.callout-note}
Obviamente, podemos obtener un resultado similar usando *raycasts*, pero en este caso podría ser un poco excesivo, ya que se puede resolver el problema de forma más eficiente con cálculos trigonométricos sencillos.
:::
