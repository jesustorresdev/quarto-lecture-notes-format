---
abstract: |
  Perforce es un sistema de control de versiones centralizado que escala muy bien para manejar proyectos y equipos realmente grandes.
  En este capítulo se explica cómo funciona Perforce, sus ventajas frente a otros sistemas de control de versiones y cómo se puede utilizar en un estudio de videojuegos.
---
...
# Control de versiones Perforce {#sec-perforce}
...
![[Going Remote: Build Up Your Game Dev Team](https://www.perforce.com/webinars/vcs/going-remote-build-your-game-dev-team) --- Robert Cowham.](/assets/perforce/helix_core_scalability.jpg){#fig-helix-code-scalability}
...
Por ejemplo, veamos en la @tbl-git-large-projects el tamaño y los tiempos --obtenidos sin ningún rigor, desde mi propia casa-- para algunos proyectos conocidos.
...
: Tiempos y tamaños de los repositorios Git de algunos proyectos conocidos. {#tbl-git-large-projects}
...
Esto quiere decir que Perforce necesita un servidor, que aloja los repositorios con los archivos de los proyectos, y que cada usuario tiene que tener un cliente en su ordenador que se conecta a ese servidor para descargar los archivos y subir revisiones, tras haberlos modificado, como se ilustra en la @fig-centralized-model.

![Relación entre cliente y servidor con versionado centralizado.](/assets/perforce/centralized_model.png){#fig-centralized-model}
...
Incluso puede establecer permisos según la ruta de archivos y directorios, usando {{< var perforce.filespecs >}} con comodines, como se ilustra en la @tbl-perforce-filespecs.
...
: Ejemplos de especificadores de ruta en Perforce. {#tbl-perforce-filespecs}
...
Perforce tiene una lista bastante extensa de clientes, como se muestra en la @tbl-perforce-clients.
...
: Lista de clientes de Perforce {#tbl-perforce-clients}
...
* Los archivos descargados se marcan, generalmente, como de solo lectura.
  Esto obliga a que, para modificar un archivo, debamos indicar al cliente nuestra intención mediante la acción {{< var perforce.checkout >}} de P4V o el comando `p4 edit`.
  El cliente comprueba en el servidor si el archivo está bloqueado.
  Si no es así, lo bloquea en el servidor y cambia el archivo en el *workspace* al modo lectura-escritura para permitir su edición.
...
::: {#vid-uso-básico-perforce}

{{< video  https://www.youtube.com/watch?v=myyR_XuqWaw >}}

Uso básico de Perforce.
:::

En el @vid-uso-básico-perforce se explica cómo conectar a un servidor de Perforce y se repasan brevemente las acciones más básicas.
Para conocer más del VCS de Perforce, lo recomendable es consultar el manual de P4V [@PerforceP4VUserGuide].

![Hoja de referencia rápida de P4V.](/assets/perforce/perforce_helix_cheatsheet.png){#fig-perforce-helix-cheatsheet}
...
* Los archivos con extensión `.uasset` --archivo de recursos en el editor-- y `.umap` --archivo de escena-- también son binarios, pero en el momento que un cliente los abra para editar, automáticamente quedará bloqueada la edición para el resto de clientes --por eso el uso de `+l`--.
...
* Como hemos comentado, Perforce crea los archivos en el *workspace* donde estamos trabajando en modo solo lectura para ibligarnos a usar {{< var perforce.checkout >}} --o el comando `p4 edit`-- antes de intentar modificar cualquier de ellos.
  ...
  Por eso no se pone `l` para forzar el bloqueo.
  En su lugar se indica `w`, para que estos archivo en el *workspace* tengan permisos de escritura por defecto, pudiendo ser modificados directamente.

* Los binarios se incluyen en el control de versiones porque así el departamento de garantía de calidad, directores y otros miembros del equipo pueden probar el desarrollo rápidamente.
...
En el repositorio {{< iconify fa6-brands github >}} [ull-master-videojuegos/perforce-typemaps]( https://github.com/ull-master-videojuegos/perforce-typemaps) [@Torres2021PerforceTypemaps] se puede encontrar este mismo ejemplo de archivo de configuración de tipos tanto para Unreal Engine como para Unity.
...
En el tutorial *«Streams Overview»* [@PerforceStreams] se explica en detalle su funcionamiento, pero vamos a resumir brevemente sus principales características:
...
::: {.callout-tip}
Merece la pena ver el webinar *«Best Practices for Game Development Using Streams»* [@PerforceStreamsBestPractices] para entender mejor cómo se pueden aprovechar las características de los *streams*.
...
La instalación del cliente de Perforce incluye un servidor personal que se puede utilizar tanto para trabajar de forma aislada en proyectos personales como para trabajar en proyectos colaborativos de forma distribuida (ver la @fig-dvcs-personal-shared). 
...
![Relación entre servidores Perforce con versionado distribuido.](/assets/perforce/dvcs_personal_shared.png){#fig-dvcs-personal-shared}
