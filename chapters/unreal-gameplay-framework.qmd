---
abstract: |
  En este capítulo se describe el *gameplay framework* de Unreal Engine.
  Se explica cómo se organiza el código de un videojuego en este motor y se detallan las clases y componentes que se utilizan para implementar las mecánicas de juego.
---
...
%%| file: ../diagrams/unreal_gameplay_framework/gamemode-inheritance.mmd
```
...
En este sentido, los **GameMode** hacen de *game / level manager* --responsabilidad que puede compartir en parte con los *level blueprints* (ver la @sec-level-blueprint)---, mientras que **GameInstance** sirve para preservar información entre partidas, como el número de victorias de cada equipo --por ejemplo, para enfrentamientos del tipo «el mejor de X partidas»-- los inventarios, las habilidades o la configuración del juego y de los personajes.
...
Mientras que para desposeerlo se usa el método {{< api UnPossess >}} (ver la @fig-possess-nodes).

![Nodos de los **Controller** para poseer y desposeer un **Pawn**.](/assets/unreal-gameplay-framework/possess-nodes.png){#fig-possess-nodes}
...
El {{< api Controller >}} recibe los eventos {{< api OnPossess >}} o {{< api OnUnPossess >}} y el {{< api Pawn >}} recibe los eventos {{< api Possessed >}} o {{< api UnPossessed >}} (ver la @fig-controller-possess-events y la @fig-pawn-possess-events).

::: {#fig-ue-input-action layout="[[1], [-1], [1]]"}
![Eventos de posesión de **Controller**.](/assets/unreal-gameplay-framework/controller-possess-events.png){#fig-controller-possess-events}

![Evento de posesión de **Pawn**.](/assets/unreal-gameplay-framework/pawn-possess-events.png){#fig-pawn-possess-events}

Nodos de eventos de posesión.
:::
...
%%| file: ../diagrams/unreal_gameplay_framework/pawn-control.mmd
```
...
#### Movimiento del actor {#sec-movimiento-del-actor}
...
%%| file: ../diagrams/unreal_gameplay_framework/pawn-movement-sequence.mmd
```
...
%%| file: ../diagrams/unreal_gameplay_framework/character-movement-sequence.mmd
```
...
## Gestión de la entrada del jugador (*PlayerController*) {#sec-playercontroller}
...
Como se esquematiza en el @vid-unreal-input-component-stack, según va pasando por los **InputComponent**, cada uno de ellos puede elegir «consumir» el evento de entrada, haciendo que no alcance a componentes posteriores en la pila.

::: {#vid-unreal-input-component-stack}
{{< video /assets/unreal-gameplay-framework/input_component_stack.mp4 >}} 

La pila de **InputComponent** en el **PlayerController**.
:::
...
El @vid-input-component-stack también ilustra como cualquier actor puede colocar su **InputComponent** al principio de la pila, usando el método {{< api EnableInput >}}.
...
%%| file: ../diagrams/unreal_gameplay_framework/aicontroller-classes.mmd
```
...
%%| file: ../diagrams/unreal_gameplay_framework/ue-character-movement-hierarchy.mmd
...
Por lo general, la percepción de los NPC --que pueden ver u oír-- se implementa utilizando *raycasts* o *triggers* (ver el @sec-colisiones-triggers-raycasts).
...
* El {{< api WheeledVehiclePawn >}} extiende **Pawn** para implementar la clase base de vehículo con ruedas.
  ...
  Esta clase de actor incluye por defecto un componente {{< api ChaosWheeledVehicleMovementComponent >}} que implementa la simulación del movimiento del vehículo, usando el motor de físicas Chaos de Unreal Engine (ver el @sec-simulacion-fisica).
...
### Personalizar el comportamiento de la cámara {#sec-camera-customization}
...
```{mermaid}
%%| label: fig-update-camera
%%| fig-cap: Esquema de actualización del POV del jugador.
%%| file: ../diagrams/unreal_gameplay_framework/update-camera-sequence.mmd
```
...
%%| file: ../diagrams/unreal_gameplay_framework/ue-gameplay-framework.mmd
```
