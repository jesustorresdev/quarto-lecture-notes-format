---
abstract: |
  El **motion matching** es una técnica de animación que se basa en la selección de *keyframes* de animación en función del estado del personaje y de su entorno, para crear animaciones fluidas y realistas.
  En este artículo se describen los principales componentes y características de esta técnica.
---

{{< audio file="../assets/motion-matching/motion-matching.mp3" caption="Podcast autogenerado del capítulo." download_link="true" >}}
...
Una de las técnicas más comunes para animar personajes en tiempo real, es el uso de varios *clips* de animación por *keyframes* y una máquina de estados que se encarga de seleccionar el *clip* de animación que se debe reproducir en cada momento.
...
Esta selección se hace varias veces por segundo y el motor de videojuegos se encarga de saltar y mezclar los *clips*, sintetizando la animación del personaje.

Esta técnica evita crear una máquina de estados compleja.
...
![Posición de los pies.](/assets/motion-matching/feature-position.png){#fig-feature-position}

![Velocidad articulaciones.](/assets/motion-matching/feature-velocity.png){#fig-join-velocity}

![Trayectoria general.](/assets/motion-matching/feature-trajectory.png){#fig-feature-trajectory}
...
Las propiedades no solo se calculan y almacenan para cada *keyframe* de la base de datos de movimiento, sino que también se extraen para el estado actual y movimiento a partir de la entrada del jugador, con el objeto de usarlas para preparar la consulta a la base de datos.
...
La técnica de ***motion matching*** es perfectamente compatible con otras técnicas y sistemas de animación.
Por ejemplo, se puede ejecutar como un nodo dentro de una máquina de estados de animación, lo que permite transicionar entre ***motion matching*** y *clips* seleccionados de forma tradicional o *blend spaces*.
También se pueden introducir etapas posteriores para ajustar la animación de forma procedural, usando *cinemática inversa* o *simulación física*.
...
En el @vid-learned-motion-matching se ilustra la técnica descrita en un diagrama de bloques.

::: {#vid-learned-motion-matching}
{{< video /assets/motion-matching/learned-motion-matching.mp4 >}}

Diagrama de bloques de *learned motion matching*.
:::
...
En Unreal Engine, por otro lado, se incluyó, también de forma experimental, desde la versión 5.1 y de forma estable desde la versión 5.4 [@UnrealMotionMatching].
