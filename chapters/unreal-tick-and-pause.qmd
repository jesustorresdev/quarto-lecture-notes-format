---
abstract: |
    En este capítulo se explica cómo se propagan los *ticks** de actualización en Unreal Engine y cómo se pueden desactivar actores y componentes y poner el juego en pausa.
---
...
![Propiedades de la sección **Actor Tick** de un actor.](/assets/unreal-tick-and-pause/actor-tick-properties.png){#fig-actor-tick-properties}
...
Por ello, las *funciones de tick* también pueden tener *prerrequisitos*, que el motor asegura que se cumplen antes de ejecutar el *tick*.
...
### Actualización {#sec-tick-activation}
...
## Escalado de tiempo {#sec-time-scaling}

El escalado de tiempo es una técnica que se utiliza para ralentizar o acelerar temporalmente el paso del tiempo, sin afectar a la lógica del juego.
Por ejemplo, se puede usar para ralentizar el tiempo cuando el jugador está a punto de ser golpeado por un enemigo, o para acelerarlo cuando está viajando a través de un mapa muy grande.

En Unreal Engine, la escala global de tiempo se guarda en una propiedad del **UWorld** que se puede modificar usando {{< api SetGlobalTimeDilation >}}.
El valor por defecto es 1.0, que indica que el tiempo transcurre a velocidad normal, pero un valor de 0.5 ralentiza el tiempo a la mitad, mientras que un valor de 2.0 lo acelera al doble.
Esto afecta a la velocidad a la que reciben *ticks* actores y componentes, pero se debe tener en cuenta que no tiene efecto en la simulación del motor de física.
Por tanto, el escalado de tiempo no afecta al movimiento de los componentes con la simulación de física activada.

Además, cada actor tiene una propiedad **Custom Time Dilation** que permite escalar el tiempo para él y sus componentes de forma independiente.
Este escalado se hace de forma relativa a la escala de tiempo global.
Por ejemplo, se puede crear la ilusión de que un personaje se mueve muy rápido, poniendo el juego globalmente en *slow motion* usando un **Global Time Dilation* de 0.1, pero poniendo el **Custom Time Dilation** del actor del personaje a 10.
