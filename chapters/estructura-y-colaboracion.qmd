---
abstract: |
  En este capítulo se presentan algunas recomendaciones para organizar el proyecto y facilitar la colaboración entre los diferentes equipos involucrados.
---

{{< audio file="../assets/estructura-y-colaboracion/estructura-y-colaboracion.mp3" caption="Podcast autogenerado del capítulo." download_link="true" >}}
...
Poner algo de orden en un proyecto es importante para asegurar su sostenibilidad.
Facilita que cada miembro del equipo sepa donde buscar o añadir aquello que necesite para hacer su trabajo.
Por eso, en los siguientes apartados veremos algunas recomendaciones para organizar el proyecto y facilitar la colaboración entre los diferentes equipos involucrados.
...
* En **Open 3D Engine** los proyectos se ubican en el directorio `dev` dentro del árbol de directorios del motor.
  El código fuente del proyecto se guarda dentro de `dev/code/<NombreDelProyecto>`.
  Mientras que los recursos --incluidos los *scripts* en Lua-- se almacenan en `dev/<NombreDelProyecto>`.

* En **Godot** se puede organizar libremente los recursos en la carpeta del proyecto.
  Al acceder a los archivos, se recomienda usar `res://` para referenciarlos, de tal forma que se pueda mover el proyecto sin tener que cambiar las rutas.
...
%%| file: ../diagrams/estructura-y-colaboracion/unity-assets-tree-by-type-a.mmd
```
...
%%| file: ../diagrams/estructura-y-colaboracion/unity-assets-tree-by-type-b.mmd
```
...
%%| file: ../diagrams/estructura-y-colaboracion/unity-assets-tree-by-type-c.mmd
```
...
```{mermaid}
%%| file: ../diagrams/estructura-y-colaboracion/unity-assets-tree-by-feature.mmd
```

```{mermaid}
%%| file: ../diagrams/estructura-y-colaboracion/unity-assets-tree-by-feature_part1.mmd
```
...
::: {.callout-tip}
Para ayudar a localizar rápidamente diferentes tipos de *assets*, puede ser útil asignar diferentes colores a las carpetas, según su contenido.
En Unreal Engine, por ejemplo, se pueden asignar colores usando la opción **Set Color** del menú contextual, que se abre al hacer clic con el botón derecho del ratón sobre la carpeta.
Mientras que en Unity se suele recurrir a algún *plugin* del *Asset Store* que permita hacer lo mismo, como *«Rainbow Folders 2»*[@BorodarRainbowFolders].
:::
...
En Unreal Engine lo más parecido son los Blueprints, que muchas veces se guardan en la carpeta ``.
Sin embargo, puede ser buena idea separar los Blueprints y otros *assets* que los diseñadores usan para componer los niveles, de los Blueprints que implementan la lógica del juego --como *game modes*, controladores, clases base y componentes--.
Para lo primero se puede usar una carpeta `LevelContento` o `Placeables` y para lo segundo la carpeta `Blueprints`.
...
%%| file: ../diagrams/estructura-y-colaboracion/unity-gameassets.mmd
```
...
Sin embargo, los paquetes adquiridos en la web de *Asset Store* se descargan e instalan mediante el **Unity Package Manager** --a partir de Unity 2020.x-- pero se siguen importando en `Assets`, por lo que debemos seguir teniendo cuidado.
:::

Una práctica interesante es colocar un `'_'` en el primer carácter del nombre de la carpeta.
...
* **Facilita controlar qué código se incluye o deja de incluir según la plataforma del proyecto**, ya que eso se puede indicar en la definición de cada *assembly*.
  También permite controlar qué código es para el editor y cuál es para el *gameplay*, sin tener que ponerlo en un directorio con un nombre concreto --como `Editor`, que es dónde Unity espera encontrar los *scripts* del editor--.
...
Esta carpeta se usa para archivos que queremos que estén disponibles directamente en el sistema de archivos del dispositivo donde se instala el juego, sin las conversiones que hace Unity, accesibles mediante una ruta de archivo convencional.

### Adoptar *AssetBundles* en lugar de `Resources`

En realidad, el empleo de la carpeta `Resources` puede presentar algunos problemas por lo que se recomienda no usarla [@UnityResourcesFolder].
Excepto, tal vez, para un conjunto mínimo de componentes que necesite el arranque del juego.

En lugar de `Resources`, es preferible utilizar **AssetBundles**[@UnityAssetBundles], que son una forma de empaquetar recursos para cagarlos y descargarlos durante la ejecución, de forma selectiva.
Gracias al **Addressables Assets System** [@UnityAddressables] se puede asignar a cada **AssetBundle** una dirección única que durante la ejecución sirve para localizarlos y cargarlos en la aplicación, independientemente de dónde estén ubicados
...
Sin embargo, lo que interesa destacar es la posibilidad de crear nuestros propios paquetes y usarlos como dependencia de nuestros proyectos [@UnityCreateCustomPackage].
...
En la documentación de Unreal Engine [@UnrealAssetNameConventions] se indica una lista de prefijos recomendados para los diferentes tipos de *assets*.
En el repositorio *«Gamemakin UE4 Style Guide()»* [@AllarAssetNamingConventions] se recopila una lista mucho más extensa de prefijos y sufijos sugeridos, que puede usarse para complementar a la anterior.
...
%%| file: ../diagrams/estructura-y-colaboracion/ue-assets-tree-by-feature.mmd
```
...
* **En carpetas como `Content/Characters`, `Content/Weapons` o `Content/Pickups` se suelen organizar subdirectorios para los distintos personajes, armas o recolectables, respectivamente**.
  En algunos proyectos se optar por poner estas carpetas dentro de `Content/LevelContent`, `Content/Placeables` o similares.
...
### Relación entre clases de Blueprint y C++

Aunque en muchos cursos y tutoriales todo se desarrolla en Blueprint para que sea más accesible, no es así como se suelen desarrollar.

Por norma general los actores y componentes de los sistemas del juego se desarrollan en C++, por lo que son responsabilidad del equipo de programadores.

Estas clases generalmente exponen una interfaz pública de métodos, propiedades y eventos accesible desde Blueprint.
En el editor se hereda de ellas para crear clases Blueprint, lo que facilita a los diseñadores poder configurar, ajustar el comportamiento, posicionar y conectar actores directamente en editor.

### Módulos de C++

Para facilitar la sostenibilidad y la reutilización del código es recomendable implementar sistemas diferentes en módulos separados [@UnrealModules].
...
Si alguno de esos módulos tenía una dependencia que no es también dependencia de los **módulos de gameplay**, la dependencia tampoco es compilada e incluida en el proyecto final.
...
En el caso de que los módulos sean lo bastante generales como para poder ser reutilizados en otros proyectos, podríamos incluirlos en *plugins* [@UnrealPlugins].
...
Esta arquitectura uitliza de forma intensiva dos *plugins* que vienen con Unreal Engine 5: **Game Features** y **Modular Gameplay**.
...
%%| file: ../diagrams/estructura-y-colaboracion/scene-hierarchy.mmd
```
...
%%| file: ../diagrams/estructura-y-colaboracion/unity-additive-scenes.mmd
```

En la @fig-unity-additive-scenes se puede observar un ejemplo dónde los elementos generales del nivel --`Level01`-- los elementos específicos de la misión actual --`Quest01`-- y la interfaz de usuario --`UI`-- están en escenas diferentes.
...
En Unreal Engine se puede hacer algo similar usando subniveles, que son escenas que se cargan y descargan en tiempo de ejecución (ver la @sec-ue-sublevels)
...
Para eso es fundamental contar con herramientas de control de versiones --en inglés, *Version Control System* o VCS-- como Git, {{< var perforce.helixcore >}} o {{< var unity.vcs >}}.

En desarrollo de videojuegos, herramientas como Unity VCS y Perforce --que es considerado el estándar *de facto* de la industria-- tienen una ventaja importante respecto a Git en que tienen la capacidad de almacenar y versionar los recursos del juego, contenidos de arte y documentos de diseño, además del código fuente.

::: {.callout-note}
En Git se pueden conseguir características similares a las de otras herramientas de control de versiones utilizando {{< var git.lfs >}}.
:::
...
El ejemplo más común es {{< var shotgrid >}}, pero Perforce también tiene el suyo, llamado {{< var perforce.helixdam >}}.
Gracias a las facilidades que ofrece un DAM, el director creativo y los *lead artists* puede ofrecer *feedback* en el estilo que se busca y pedir cambios y mejoras, al tiempo que se mantiene un registro de todas las pruebas que se hacen a nivel artístico.

En los estudios más pequeños no suele compensar el coste extra de un DAM.
Por eso se suele optar por utilizar para eso el propio software de control de versiones o incluso soluciones de almacenamiento más convencionales, como Dropbox o un servidor de archivos.
...
Además, en Unreal Engine se debe valorar si incluir o excluir el directorio `Content/Developers`, si por la forma en la que se colabora resulta útil que los contenidos privados que algunos desarrolladores copian en ella puedan ser vistos por todos (ver la @sec-carpeta-developers).
...
### Colaborar en el VCS {#sec-colaborar-vcs}
...
Aun así, en el @sec-perforce desarrollamos las características y ventajas de Perforce frente a otros VCS.
:::
...
En este segundo caso, la manera de colaborar es muy similar a la empleada con Unity, utilizando subniveles.

Además, el editor ofrece algunas funcionales adicionales, que también comentaremos.

#### Subniveles {#sec-ue-sublevels}
...
Luego podemos decidir cuándo cargar los subniveles en función, por ejemplo, de la posición del jugador en el mapa.
Sin embargo, si algún subnivel se marca como **Always Streaming**, ya no se cargará a demanda, sino junto al **Persistent Level**.
...
En la @fig-ue-sublevels se puede observar un ejemplo de organización de un nivel en subniveles en el panel **Levels** en el editor de Unreal Engine.

![Panel **Levels** en el editor de Unreal Engine.](/assets/estructura-y-colaboracion/ue-sublevels.png){#fig-ue-sublevels}
...
* Cada actor del nivel se guarda en su propio archivo --también conocidos por actores externos-- por lo que al editar no se tiene que bloquear el archivo del nivel, sino el archivo de cada actor que se está manipulando, lo que facilita la colaboración.
...
En la @fig-ue-datalayers se puede observar un ejemplo del panel de **Data Layers** en un nivel con **World Partition** activado.

![Panel **Data Layers** en el editor de Unreal Engine.](/assets/estructura-y-colaboracion/ue-datalayers.png){#fig-ue-datalayers}

#### Prefabs

En Unity los *prefabs* son como plantillas a partir de la cual se pueden crear instancias de objetos en la escena.
Unreal Engine no soporta *prefabs*, pero en el *marketplace* existen diferentes soluciones que sí intentan ofrecer una implementación.
En todo caso, los actores Blueprint se pueden utilizar, hasta cierto punto, para tener una funcionalidad similar.
...
Lamentablemente, a veces hay características que solo están disponibles como un actor y no como un componente que puedas poner dentro de un actor.
...
Sin embargo, debemos tener encuenta que, en general, no se recomienda usar actores hijos.
...
Realmente, las **colecciones** no guardan los *assets*, si no referencias a dichos *assets* en el proyecto.
...
![Desplegable del panel **Collections**.](/assets/estructura-y-colaboracion/collections-toggle.png){#fig-collections-toggle}
