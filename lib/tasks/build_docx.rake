...
Project::find_documents().each do |document|
    namespace "#{document[:namespace_prefix]}build" do
        ...
        task :docx => [ document[:output_pathname][:docx] ]

        file document[:output_pathname][:docx] => [:docbook, document[:output_directories][:docx]] do |t|
            pandoc_opts = CONFIG[:pandoc_docx_opts]
            sh "pandoc", '--from', 'docbook',
                         '--to', 'docx',
                         '--resource-path', document[:output_directories][:docbook],
                         *pandoc_opts,
                         '--output', t.name, document[:output_pathname][:docbook]
        end

        directory document[:output_directories][:docx]

        task :docbook => [ document[:output_pathname][:docbook], :docbook_media_files ]
        ...
        file document[:output_pathname][:docbook] => [*document[:dependencies], :config] do |t|
